/**
 * Tarifas aproximadas de transporte público en Lima Metropolitana.
 * Se usa el costo más alto de cada rango definido en la tabla de negocio.
 */
...
export const LIMA_TRANSPORT_FARES: Record<LimaTransportType, LimaTransportFare> = {
    metro_linea_1: {
        id: 'metro_linea_1',
        label: 'Línea 1 (Metro de Lima)',
        fare: 1.5,
        formal: true,
    },
    metro_linea_2: {
        id: 'metro_linea_2',
        label: 'Línea 2 (Metro de Lima)',
        fare: 1.5,
        formal: true,
    },
    metropolitano_troncal: {
        id: 'metropolitano_troncal',
        label: 'Metropolitano (Troncal)',
        fare: 3.2,
        formal: true,
    },
    metropolitano_alimentador: {
        id: 'metropolitano_alimentador',
        label: 'Metropolitano (Alimentador)',
        fare: 1.5,
        formal: true,
    },
    metropolitano_integrado: {
        id: 'metropolitano_integrado',
        label: 'Metropolitano (Integrado)',
        fare: 3.5,
        formal: true,
    },
    corredor_complementario: {
        id: 'corredor_complementario',
        label: 'Corredores Complementarios',
        fare: 2.5,
        formal: true,
    },
    bus_local: {
        id: 'bus_local',
        label: 'Bus / Combi Tradicional (Local)',
        fare: 2.0,
        formal: false,
    },
    bus_interdistrital: {
        id: 'bus_interdistrital',
        label: 'Bus Tradicional (Interdistrital)',
        fare: 3.5,
        formal: false,
    },
    bus_cono_a_cono: {
        id: 'bus_cono_a_cono',
        label: 'Bus Tradicional (Cono a Cono)',
        fare: 5.5,
        formal: false,
    },
    colectivo: {
        id: 'colectivo',
        label: 'Auto Colectivo / Informal',
        fare: 7.0,
        formal: false,
    },
    mototaxi: {
        id: 'mototaxi',
        label: 'Mototaxi de aproximación',
        fare: 1.5,
        formal: false,
    },
};

/** Distritos periféricos donde aplica mototaxi en tramos a pie largos. */
...
export function isPeripheralDistrict(district?: string): boolean {
    ...
    if (!key) return false;
    return PERIPHERAL_DISTRICTS.has(key);
}
...
function stepText(step: DirectionsTransitStep): string {
    ...
    const parts = [
        ...
        line?.vehicle?.name,
        line?.vehicle?.type,
        line?.agencies?.map(a => a.name).join(' '),
        line?.color,
...
export function classifyTransitStep(step: DirectionsTransitStep): LimaTransportType | null {
    ...
    if (/l[ií]nea\s*1|\bl1\b|metro.*1|tren verde/.test(text)) {
        return 'metro_linea_1';
    }
    if (/l[ií]nea\s*2|\bl2\b|metro.*2/.test(text)) {
        return 'metro_linea_2';
    }
    ...
    if (/corredor\s*(complementario|rojo|azul|morado|amarillo)|\bcr[\d-]/.test(text)) {
        return 'corredor_complementario';
    }

    if (vehicleType === 'TAXI' || /colectivo/.test(text)) {
        return 'colectivo';
    }
...
export function getDefaultTransportFaresList(): LimaTransportFare[] {
    return Object.values(LIMA_TRANSPORT_FARES);
}

export function buildTransportFaresMap(
    overrides: TransportFareSetting[] = []
...
export function resolveTransportFares(
    settings?: { transportFares?: TransportFareSetting[] } | null
): Record<LimaTransportType, LimaTransportFare> {
    if (!settings?.transportFares?.length) return LIMA_TRANSPORT_FARES;
    return buildTransportFaresMap(settings.transportFares);
}

export function resolveTransportFaresList(
    settings?: { transportFares?: TransportFareSetting[] } | null
): LimaTransportFare[] {
    const map = resolveTransportFares(settings);
    return getDefaultTransportFaresList().map(f => map[f.id]);
}
...
export function calculateRouteCostFromSteps(
    ...
    for (const step of steps) {
        ...
        if (mode === 'WALKING') {
            ...
            if (dist >= 500 && isPeripheralDistrict(originDistrict)) {
                mototaxiSegments++;
            }
            continue;
        }
    ...
    const otherTypes = transitTypes.filter(
        t => t !== 'metropolitano_alimentador' && t !== 'metropolitano_troncal'
    );
...
export function formatRouteCostDisplay(amount: number | null | undefined): string {
